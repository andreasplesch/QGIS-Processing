...
if crsProjection not in X3DProjections.keys():
    raise GeoAlgorithmExecutionException('%s projection not supported by X3D. Reproject first.' % crsProjection)
...
# origin
e = r.extent()
geoGridOrigin = [e.xMinimum(), e.yMinimum(), 0]
...
f.write( "  geoGridOrigin='%f %f %f'\n" % (geoGridOrigin[0], geoGridOrigin[1],  geoGridOrigin[2]) )
